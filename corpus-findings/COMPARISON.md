...
## The grammar learns a construct, and Exposed gains 723 declarations

The Kotlin bake-off (DECISIONS, same date) put the pinned grammar last of five on
every measure that matters, and the largest single cause was one construct.
Bisecting `Entity.kt` by lines, the first prefix whose parse discards any text
ends at `is EntityID<*> if reference.referee<REF>() == …` — a `when` guard,
Kotlin 2.1, which `tree-sitter-kotlin-ng 1.1.0` does not know. One guard ends
the grammar's reading of the whole file.

So the grammar learns it. `vendor/tree-sitter-kotlin-ng/` is the 1.1.0 release
plus one `choice` arm in `when_entry` — a guard binds ONE condition, which is
also what keeps it apart from an `if` expression opening the next condition
after a comma — with `grammar.json`, `node-types.json`, `parser.c` and
`tree_sitter/parser.h` regenerated by `tree-sitter-cli 0.25.10`, the version
this workspace links as its runtime. The provenance record names all five and
`vendored_trees_are_upstream_plus_patches` checks them.

What it buys, over Exposed's 860 `.kt`/`.kts` files:

| | before | after |
|---|---|---|
| files with a parse error | 61 | 49 |
| non-space bytes no node covers | 147100 | 98000 |
| declaration nodes | 21809 | 22532 |

**Exposed: 963 → 977.** Twenty findings move, and the direction of each says
what came back.

- `+17 duplicate`, sixteen of them in `exposed-core` and most in
  `ColumnType.kt`: the `valueFromDB` and `equals` overrides that column types
  repeat. They were never absent from the source — they sat under an `ERROR`,
  and a clone needs both halves visible. `OpBuilder.kt`, the file that discarded
  the most text of any in the corpus (24560 non-space bytes), supplies the
  seventeenth.
- `−2 unused`. `Entity.kt:isPersistedIn` is the clearer one: it was reported
  dead as an OWNERLESS declaration, because the class it belongs to was not a
  node at all. It now reads as `Entity.isPersistedIn` and `used-by` names its
  keeper — a reference at `Entity.kt:325`, in text the old parse discarded.
  `References.kt:allReferencesMatch` is the same shape.
- `−1 internal-only` on `LocalDateTimeColumnType.kt:toSqlTimestamp`, for the
  same reason in the other direction: with the file parsed, the uses that
  decide its rung are visible.

The other eight repositories are byte-identical.

**Two fixtures move with it.** `when-guard-grammar-gap` is renamed `when-guard`:
its `known_gap` closes, `Waste.kt` is reported dead where the run used to abstain
whole-run for want of a root, and a second row now pins that the guard's
expression is read as an expression — `lengthy`, called only from inside the
guard, stays alive. `recovered-under-error` keeps its subject and changes its
