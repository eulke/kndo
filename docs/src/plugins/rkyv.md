...
Same shape as `kndo:serde`, same two mechanisms stopping short: rkyv's
...
to. Dispatch through generated code is invisible to source extraction by construction, so the
whole bridge reads as production-reachable and test-unreached.
...
- **Native-only.** The WIT ABI `annotate-symbols` surface doesn't carry this plugin's marks —
  the same gap `kndo:serde` has — so a WASM guest plugin has no way to reproduce them yet.
