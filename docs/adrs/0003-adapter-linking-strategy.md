...
1. **Compiled-in** — the eight launch adapters and first-party plugins are crates statically
   linked into the `kondo` binary, selected/activated at runtime.
2. **WASM components** — third-party adapters and plugins target `kondo-plugin-api`, a versioned
...
- "Pluggable" never means "recompile kondo": external languages are possible without a core PR —
