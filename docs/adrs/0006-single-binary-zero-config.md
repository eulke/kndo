...
- `kondo check` must produce a correct, useful report in any repo with **zero configuration**:
  languages auto-detected by adapters' claims, ecosystems auto-detected by plugin predicates,
  ignores inherited from `.gitignore`. `kondo.toml` only ever *tunes*.
...
- Auto-detection must be introspectable or it becomes magic: `kondo doctor` (RFC 0006 §2) is the
