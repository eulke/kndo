...
## 2026-09-09 — The grammar learns the `when` guard, and keeps its fields

The bake-off's verdict was that the pinned Kotlin grammar loses. Acting on it had
two shapes, and the first measurement of the second one is what decided between
them.

`brokk-tree-sitter-kotlin 0.4.6` reads the language best — 17 error files to 61,
3392 discarded bytes to 147100, 24602 declarations to 21809. But it is a
field-less grammar: 6 field declarations to `kotlin-ng`'s 24, and ONE name
position to its six. The grammar ledger's unit is a field whose declared types
include a name kind — the grammar itself saying "a name goes here" — and its
design excludes typed child slots on purpose, because a name under an expression
kind is not a role, it is nesting. Under brokk the Kotlin ledger would fall from
six argued positions to one, `child_by_field_name("name")` would go back to
positional scanning, and the gate that caught six of the nine-adapter audit's
findings would have nothing left to say about Kotlin. Swapping buys the language
and sells the structure.

The plan's own mechanism is the third answer, and it works: vendor the canonical
grammar and patch it. `vendor/tree-sitter-kotlin-ng/` is 1.1.0 plus one `choice`
arm in `when_entry` for Kotlin 2.1's guard — `is String if x.isEmpty() -> …` —
with `grammar.json`, `node-types.json`, `parser.c` and `tree_sitter/parser.h`
regenerated by `tree-sitter-cli 0.25.10`, the version this workspace links. The
first attempt at the rule did not generate: an `optional(seq('if', …))` after the
comma list is ambiguous with an `if` expression opening the next condition. The
rule that generates says what the language says — a guard binds ONE condition —
and the ambiguity is gone with it.

Measured on Exposed's 860 files: 61 error files → 49, 147100 discarded non-space
bytes → 98000, 21809 declaration nodes → 22532. Two regression cases are pinned
beside it and still parse: a multi-condition entry (`is String, is Int ->`) and
an `if` expression used AS a condition. Exposed's findings move 963 → 977: `+17
duplicate` (the `valueFromDB`/`equals` overrides in `ColumnType.kt`, which needed
both halves visible), `−2 unused` (`Entity.isPersistedIn`, which used to be
reported dead as an OWNERLESS declaration because its class was not a node, now
kept by a reference at `Entity.kt:325`; and `allReferencesMatch`), and `−1
internal-only`. The other eight repositories are byte-identical.

`kndo:kotlin` moves 19 → 20 — the same source, different evidence.
`when-guard-grammar-gap` becomes `when-guard`, its gap shut and a second row
pinning that the guard's expression is read as one. `recovered-under-error` moves
its break to infix `get`, which still fails: the construct a fixture pins must be
one the grammar still gets wrong, and the guard no longer is.

What is NOT decided here: the remaining 98000 bytes. Infix `get`/`set` and
context parameters are what is left, and both are more than one `choice` arm. The
