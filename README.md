...
Deterministic dead-code and health analysis for polyglot repositories: TypeScript and
JavaScript, Rust, Go, Java, Kotlin, Python, Swift, HTML and CSS in one run, over one
dependency graph. The same tree gives the byte-identical report at any thread count,
cached or not, and every finding names its evidence or abstains.

```sh
curl -fsSL https://raw.githubusercontent.com/eulke/kondo/main/install.sh | sh
kndo                      # findings, health, and what could not be judged
kndo trace src/index.ts   # why is this alive
```

The user documentation is the book under [`docs/`](docs/src/introduction.md): install,
getting started, every finding category, health and coverage, configuration,
suppressions and the baseline, CI, the navigation verbs, agents, languages, extensions.
The mechanics of contributing are in [`CONTRIBUTING.md`](CONTRIBUTING.md); the judgment
layer for working in the tree is [`CLAUDE.md`](CLAUDE.md).

## The rebuild — v2's engineering log

This tree is the kndo greenfield rebuild, built **in this repository, in place**: v1
was the read-only quarry it was harvested from — in history since the root swap of
2026-09-02 — and the `M-1` groundwork below (corpus, oracle, harvest, spikes) still
anchors every measurement. Product name: **kndo** (confirmed 2026-08-29).
...
The workspace is the repository root (v1 lives in history since the root swap of
2026-09-02); `cargo` commands run from here, on the pinned toolchain
...
- `crates/kndo-gates` — the gate registry `.github/workflows/ci.yml` is **generated**
...
- `xtask` — `gen-ci`, `package` (build + tar.gz + sha256), `verify-artifact`
  ...
  install check).
- CI (`ci.yml`): lint (fmt, clippy, conventional-commit), test matrix on linux/macos
...
- CI verification: GitHub Actions is billing-blocked for this repository — every
  `ci.yml` run on the branch (79 through 2026-09-01 20:20 UTC, past the credit
...
- **M7.f — the product cortex** — **shipped 2026-09-02**: the docs site
  ...
  deployed to Pages from `main`); the release surface rendered from one table
  (`kndo_gates::release` → `release.yml`, `xtask package`/`verify-artifact`,
...
- **M7.g — the root swap** — **shipped 2026-09-02**: v2 is the repository root,
  renamed into place so history follows every file; v1 stays in history. The
  release workflow's `v*` tag trigger and the docs site's Pages deploy from `main`
  are live — both exercised by CI on every push before the first tag.
