...
| adapters: css, html, json (v1 claims 9 territories) | absent — v2 claims 7 (python is v2-only; v1 never had it) | open, swap-blocking for mixed web trees (2026-09-01 audit): COMPARISON already measures the missing HTML/CSS edge set as the playground file-accusation gap on vite; json's actual consumer needs its own demand check before porting |
| coverage ingesters beside lcov: cobertura, jacoco, go-cover | lcov alone shipped | open (2026-09-01 audit): jacoco is the JVM's native format and go-cover Go's — `untested` under-serves those ecosystems until each is ported with a real captured fixture or measured dead |
| built-in conduct plugins: nextjs, express, serde, info-plist, thymeleaf, libsass, uikit, rkyv, wasmtime — each with a baseline-then-plugin proof | none built in (the `.kndo/plugins/` door + lcov only) | open, swap-blocking for users of those frameworks (2026-09-01 audit): without the keep-alive roots their findings RETURN as false positives — the exact silent-return failure v1's `builtin_plugin_proofs` gate exists to catch; each lands only through the reflection-dispatch experiment rule recorded above |
| `docs/` mdbook site (install, getting-started, languages, configuration, health, navigation, agents, ci, faq) + `docs.yml` | absent | open, swap-blocking (2026-09-01 audit): shipped user-facing surface; rebuilt against v2's real behavior, never transliterated |
| release channels: `release.yml` (git-cliff body), `install.sh`, the GitHub Action, the Homebrew template — held consistent by v1's `release_channels` test | `xtask package` + `verify-artifact` + the CI matrix exist; no channel is wired | open, swap-blocking (2026-09-01 audit): the root swap must re-point every consumer at v2's artifact and carry the one-producer test law |
| `cargo xtask bench` (machine-specific baseline in CONTRIBUTING) and `gen-stdlib` (stdlib datasets feeding v1 resolution) | absent | open (2026-09-01 audit): bench is the perf-regression instrument and wants a v2 baseline; gen-stdlib's job (bare-specifier classification) has no measured v2 gap traced to it — verify at swap, then port or declare dead with the number. First head-to-head numbers (2026-09-01, release builds, this container, cwd-identical invocations, JSON to /dev/null, 3+ runs): cold `--no-cache` — guava v1 17.8s / v2 9.1s, vite v1 1.8s / v2 0.4s (v1 claims 434 more files there: json 46 + css 233 + html 155), ripgrep 0.47 / 0.30, gin and Exposed at parity; warm over the on-disk cache — guava v1 unstable at 8.9–31.9s across runs (spikes above its own cold) vs v2 stable 1.0–1.2s; peak RSS on guava cold — v1 2,999 MiB vs v2 176 MiB (17×). flask is not comparable (v1 never had a python adapter). Machine-specific like v1's CONTRIBUTING baseline — direction, not gospel |
| Windows in the CI test matrix | deferred by owner decision (DECISIONS: tree-sitter-scss upstream, an obstacle v2 no longer carries) | open, swap-blocking (2026-09-01 audit): v1 verifies Windows on every push; reinstate at or before the swap |
