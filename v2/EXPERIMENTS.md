...
| navigation verbs (`find`/`describe`/`uses`/`used-by`/`trace`/`impact`, batched `query`) | designed + first four verbs shipped (2026-08-31) | one contract (`kndo-query/1`, `Snapshot::query`) answered from the SAME index `unused` judged on — gate-certified: used-by empties exactly where unused accused; trace/impact/explain and the serve tools are the in-flight remainder; JSONL batch stays out (serve is the batch amortizer) |
