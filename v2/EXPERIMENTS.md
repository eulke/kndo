...
| `docs/` mdbook site (install, getting-started, languages, configuration, health, navigation, agents, ci, faq) + `docs.yml` | shipped (2026-09-02): `docs/` — fourteen pages (introduction, install, getting-started, cli, findings, health, configuration, suppressions, ci, navigation, agents, languages, extensions, faq) written from v2's shipped surface, every example a real render; a `docs` CI job builds it on every push, deployment lands with the swap | closed: the install page is a release-channel consumer read by `release_channels.rs` against the target table; every relative Markdown link in the tree is held by the `every_relative_markdown_link_resolves` gate; v1's per-plugin pages have no successor — the two Apple plugins and the four ingesters are rows on the extensions page, and a page per plugin was prose about code that a proof now pins |
