...
renders `.github/workflows/v2.yml` and `v2-release.yml` from the registry; a gate fails
...
Until the root swap, a release is the `v2 release` workflow dispatched with a tag. It
packages every row of the target table through `cargo xtask package`, publishes the
