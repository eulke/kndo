...
rules as the local pre-commit hook. The Action is a **frontend**: it installs the
...
| `version` | `latest` | Release tag to install, `latest`, or `source` (build `kndo-cli` from the checkout — pre-release) |
| `working-directory` | `.` | Project root to analyze |
| `github-token` | workflow token | Token for the sticky comment |

## Outputs

`exit-code` — kndo's own exit semantics: `0` clean, `1` findings at/above
...
- **Fork PRs**: the default token cannot write comments; the Action degrades, never fails —
  annotations and the job summary still publish, and the skip is noted in the summary.
