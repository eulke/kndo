...
//!
//! Every adapter classifies files on the two orthogonal axes (role × origin).
...
/// One line rule for content-derived generated-origin detection.
#[derive(Debug)]
pub enum LineMarker {
    ...
    /// The line contains the substring anywhere *and* the line itself looks like a comment —
    /// "looks like a comment" meaning it starts with one of `ContentMarkers::comment_openers`
    /// (`@generated` and banner-style markers, which real emitters wrap in every comment shape
    /// imaginable). The comment-likeness check is load-bearing, not decorative: every adapter
    /// that scans for these markers also *documents* them somewhere in its own source as a
    /// string literal (an adapter's `GENERATED_MARKERS` const, a test fixture) — without it, a
    /// codebase that implements generated-file detection would misclassify its own detector's
    /// source as generated the moment a sibling adapter's Rust-source scanner reads it (one
    /// adapter's `Contains("@generated")` literal matches another adapter's identical marker).
    /// The opener set is adapter-supplied data, not toolkit-hardcoded (the "data from the
    /// adapter, mechanism in the core" rule) — a language whose comment syntax isn't `//`/`/*`
    /// declares its own openers in its `ContentMarkers` value instead of the toolkit guessing.
    Contains(&'static str),
}

/// An adapter's content-derived generated-origin conventions, as data — the
...
pub struct ContentMarkers {
    ...
    /// Prefixes (after trimming leading whitespace) that mark a line as a comment, for
    /// [`LineMarker::Contains`]'s self-reference guard. The adapter's own language, not the
    /// toolkit's guess — a C-family adapter supplies `&["//", "/*", "*"]`; an adapter using
    /// only [`LineMarker::PrefixSuffix`] (already self-safe by column-anchoring) can leave this
    /// `&[]`, since it's never consulted for that variant.
    pub comment_openers: &'static [&'static str],
}
        ...
        content
            ...
            .any(|line| {
                self.generated_markers
                    .iter()
                    .any(|m| self.marker_matches(m, line))
            })
    }

    fn marker_matches(&self, marker: &LineMarker, line: &str) -> bool {
        match marker {
            LineMarker::PrefixSuffix { prefix, suffix } => {
                line.starts_with(prefix) && line.ends_with(suffix)
            }
            LineMarker::Contains(needle) => self.looks_like_comment(line) && line.contains(needle),
        }
    }

    fn looks_like_comment(&self, line: &str) -> bool {
        let trimmed = line.trim_start();
        self.comment_openers.iter().any(|o| trimmed.starts_with(o))
    }
}
...
mod tests {
    ...
    const GO_STYLE: ContentMarkers = ContentMarkers {
        ...
        // PrefixSuffix never consults comment_openers — left empty on purpose.
        comment_openers: &[],
    };
    ...
    #[test]
    fn contains_ignores_a_non_comment_line_even_when_the_needle_appears() {
        // The same self-reference problem PrefixSuffix already guards against (see the test
        // above), but for Contains: an adapter's OWN marker-definition source (e.g.
        // `LineMarker::Contains("@generated")` inside a Rust const) contains the needle as
        // plain code, not a comment banner — without the comment-likeness check, one
        // adapter's GENERATED_MARKERS const is misclassified as generated by a sibling
        // adapter's identical `Contains("@generated")` marker scanning its source.
        let js = ContentMarkers {
            generated_markers: &[LineMarker::Contains("@generated")],
            scan_window_lines: 64,
            comment_openers: &["//", "/*", "*"],
        };
        let src = b"    LineMarker::Contains(\"@generated\"),\nfn f() {}\n";
        assert!(!js.detect_generated(src));
    }

    #[test]
    fn comment_openers_are_adapter_data_not_a_toolkit_guess() {
        // A hash-comment language (Python/Ruby/shell-style — none of this toolkit's current
        // adapters use it, but a future one could) declares its own opener; the toolkit itself
        // has no built-in notion of what a comment looks like.
        let hash_style = ContentMarkers {
            generated_markers: &[LineMarker::Contains("@generated")],
            scan_window_lines: 64,
            comment_openers: &["#"],
        };
        assert!(hash_style.detect_generated(b"# @generated by codegen\nx = 1\n"));
        // A `//`-style line with the marker does NOT match here — this adapter never declared
        // `//` as one of its openers, proving the set is genuinely adapter-supplied rather than
        // silently inherited from some shared default.
        assert!(!hash_style.detect_generated(b"// @generated by codegen\nx = 1\n"));
    }
}
