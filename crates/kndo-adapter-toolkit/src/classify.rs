...
pub enum LineMarker {
    ...
    /// `Contains("@generated")` literal matched the Rust adapter's identical marker).
    Contains(&'static str),
}

/// Common single-line and block-comment openers across every adapter this toolkit currently
/// serves (`//`/`/*`/`*` — the C-family syntax JS/Go/Rust/Java/Kotlin all share). Intentionally
/// small: widen it only when a real adapter with a different comment syntax needs `Contains`-
/// style marker detection (CSS also uses `/* */`; JSON has no comments, so this scanner is
/// never wired for it).
const COMMENT_OPENERS: &[&str] = &["//", "/*", "*"];

fn looks_like_comment(line: &str) -> bool {
    let trimmed = line.trim_start();
    COMMENT_OPENERS.iter().any(|o| trimmed.starts_with(o))
}
        ...
        content
            ...
            .any(|line| {
                self.generated_markers.iter().any(|m| match m {
                    ...
                    LineMarker::Contains(needle) => {
                        looks_like_comment(line) && line.contains(needle)
                    }
                })
            })
    }
