...
pub enum LineMarker {
    ...
    /// source as generated the moment a sibling adapter's Rust-source scanner reads it (found
    /// via kndo dogfooding on itself: `kndo-adapter-java/src/extraction.rs`'s own
    /// `Contains("@generated")` literal matched the Rust adapter's identical marker). The
    /// opener set is adapter-supplied data, not toolkit-hardcoded (RFC 0012 §2's "data from the
...
mod tests {
    ...
    fn contains_ignores_a_non_comment_line_even_when_the_needle_appears() {
        ...
        // plain code, not a comment banner — found via kndo dogfooding on itself
        // (kndo-adapter-java/src/extraction.rs's own GENERATED_MARKERS const was
        // misclassified as generated by the Rust adapter's identical `Contains("@generated")`
        // marker scanning its own source).
        let js = ContentMarkers {
            generated_markers: &[LineMarker::Contains("@generated")],
            scan_window_lines: 64,
            comment_openers: &["//", "/*", "*"],
