...
//! core logic). Not covered here (deliberately, not silently missing):
//! class/interface members, JSX references, string-literal subscripts
//! as precise possible-references (folded into the opaque case — see
...
/// JS/TS generated-file conventions (first-comment markers): the
/// `@generated` marker (Facebook/Metro, GraphQL codegen, relay), explicit AUTO-GENERATED
/// banners, and Go-style `Code generated by … codegen` banners that protobuf/grpc emitters
/// reuse for their JS output. `Contains` rather than anchored — real emitters wrap these in
/// every comment shape (`//`, `/* */`, `/** */`, mid-block lines). The `.d.ts`-sibling rule
/// is a documented gap: it needs cross-file knowledge no single-file hook has.
const GENERATED_MARKERS: kndo_adapter_toolkit::classify::ContentMarkers =
    kndo_adapter_toolkit::classify::ContentMarkers {
        generated_markers: &[
            ...
            kndo_adapter_toolkit::classify::LineMarker::Contains("AUTO-GENERATED"),
...
fn visibility(exported: bool) -> VisibilityLevel {
    // Ladder: module-local(0) < exported(1) < package-surface(2). The package
...
/// Everything before the body block: name, parameters and return-type
/// annotation — the declaration's *promise*, distinct from its implementation. `None` when
/// the grammar has no `body` field. Callables only;
...
/// JS/TS metric-relevant node kinds (machinery in
...
fn handle_export_statement(node: Node, src: &[u8], out: &mut FileFacts) {
    ...
    if let Some(value) = node.child_by_field_name("value") {
        // `export default <anonymous class|function|expression>` — synthetic
...
/// `export ... from "specifier"` — a re-export (barrel files are resolved through,
...
fn handle_lexical(node: Node, src: &[u8], exported: bool, out: &mut FileFacts) {
    ...
    for declarator in node.children(&mut cursor) {
        ...
        // Destructuring patterns (`const { a, b } = x`) are not extracted as declarations —
...
/// (`* as ns` — contributes no bindings here: resolving `ns.foo` back to a specific export
/// needs member-expression-aware reference resolution, which `collect_namespace_uses` owns;
/// the import edge is unaffected, only the finer per-binding fact is missed).
...
// (shapes verified via `dump_cjs_shapes` and `dump_dynamic_shapes`)

/// A `require(<single string literal>)` call — the require form that imports at `certain`.
/// The non-literal form is handled separately as a [`DynamicUse`] wildcard.
    ...
    match function.kind() {
        ...
        "import" => match single_argument(node) {
            Some(arg) if arg.kind() == "string" => {
                // `import("literal")` → probable (bundler code-split semantics). No
                // bindings: the awaited value is the namespace object, and
                // namespace member resolution is the same gap as `import * as ns`.
        ...
        // `require.resolve("literal")` — file, probable. The non-literal form is
        // not modeled (it resolves a path rather than loading a module, so folding it into
        // the require-wildcard would overstate what it keeps alive).
        "member_expression" => {
...
/// directory** the contract's `narrowed_to` expects (`./locales/${x}` → that
...
/// The generic call-site fact: any call whose callee is a plain dotted path,
...
/// (determinism over coverage), and uniformly every matching call with no
...
/// The `require("literal")` import itself — `certain`, bindings from the enclosing
...
/// CJS export surface, top-level statements only: `module.exports = …` and
/// `exports.foo = …` / `module.exports.foo = …`. Conditional exports inside blocks
/// (`if (x) module.exports = …`) are dynamic behavior — left unmodeled with the rest of the
/// dynamic constructs, not silently treated as unconditional.
///
...
///   same gap as ESM's local `export { a as b }`.
...
///   value's shape).
///
/// Computed/spread members would demote the file's export surface to `probable`, but that has
/// no mechanism — `Declaration` carries no confidence and `FileFacts::dynamics` isn't
/// consumed by assembly — so those members contribute nothing, unmodeled alongside the
...
fn handle_cjs_module_exports(
    ...
    if right.kind() == "object" {
        ...
        // declarations (certain). A member whose public name matches the local
        ...
        for member in right.children(&mut cursor) {
            let (local, skip_node) = match member.kind() {
                ...
                _ => (None, None), // computed/spread/literal members — no confidence mechanism
            };
    ...
    // false-positive `unused` (`module.exports = setup` would otherwise produce a phantom
    // dead `default` next to a live `setup`). A whole-module
...
// (shapes verified via `dump_namespace_member_shapes`)
...
/// an imported module object is out of scope. String-literal subscripts (`ns["foo"]`) are
/// folded into the computed case — strictly more conservative, the precise possible-reference
/// form needs per-binding confidence the contract doesn't carry. Second-order aliasing
...
struct NamespaceUseCtx {
    ...
    /// Bindings dedupe per (import, member) — one binding fact per name.
    seen_bindings: std::collections::HashSet<(usize, SmolStr)>,
    /// Reference dedupe folds the attribution in: one reference per
...
/// stays `None`: load-time, file-attributed. Nested declarations keep the
...
fn collect_references(
    ...
    // Attribution: outermost wins; static class-evaluation elements restore the
    ...
    {
        // `type_identifier` *is* the grammar's type-position signal: the same
        ...
        // tagged TypeUse, not Extend (heritage-clause discrimination is a gap shared with
        // class signature spans, which the leak analysis would need first).
...
mod tests {
    ...
    fn plain_reexport_of_a_local_symbol_contributes_no_import() {
        ...
        // pass-through. Out of scope here (see handle_export_statement).
    ...
    #[test]
    fn non_literal_require_is_a_wildcard_not_a_guessed_import() {
    ...
    #[test]
    fn conditional_module_exports_is_not_treated_as_unconditional() {
    ...
    fn string_call_args_record_dotted_callees_and_first_string_literal() {
        // The generic call-site fact — framework-blind, direct literals only.
        let facts = extract(
            "app.js",
            br#"
res.render("index");
...
"#,
    ...
    fn own_exports_member_read_references_the_local_symbol() {
        // exports.storage assigned once, then *read* through the
    ...
    fn escaping_exports_object_is_a_dynamic_use() {
        // `module.exports = require('./common')(exports)` — the own
    ...
    #[test]
    fn generated_markers_set_detected_origin() {
        ...
        for src in [
            ...
            assert_eq!(
                extract("f.ts", src.as_bytes()).detected_origin,
                Some(FileOrigin::Generated),
                "should detect: {src:?}"
            );
        }
        assert_eq!(
            extract("f.ts", b"export const q = 1;\n").detected_origin,
            None
        );
    }
