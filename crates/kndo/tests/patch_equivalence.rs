...
fn run_mutation(label: &str, mutate: impl Fn(&Path, &str) -> String) -> usize {
    ...
    for fixture in fixture_roots() {
        ...
        for i in 0..40 {
            fs::write(
                work.path().join(format!("kndo_filler_{i}.ts")),
                format!("export const filler{i} = {i};\n"),
            )
            .unwrap();
        }

        let sources = claimed_source_files(work.path());
        ...
        let cache = kndo_core::cache::ProjectCache::open(cache_dir.path());
        assemble_with_cache(work.path(), &adapters(), &[], Some(&cache))
        ...
        let (scratch_graph, scratch_diags) = assemble(work.path(), &adapters(), &[])
