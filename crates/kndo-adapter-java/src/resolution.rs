//! Import resolution (docs/adapters/java.md §3): pure lookups against `ResolveCtx`'s known-
...
//! (contracts §2) — never reaches this module at all. Every import specifier here is either a
...
//! `Resolution::Unresolved` — deliberately NOT a `Dependency` fallback (spec §0/§3: no
...
pub(crate) fn resolve(spec: &ImportSpec, ctx: &ResolveCtx<'_>) -> Resolution {
    ...
    // Static-import sentinel (spec §3.1): "pkg::Cls" resolves on the package half only —
...
/// declaring that `unit` (spec §3 point 2) — which one is nominal doesn't affect correctness,
...
mod tests {
    ...
    fn unresolved_third_party_never_becomes_a_dependency_edge() {
        // spec §0/§3: no reliable package->coordinate mapping — must stay Unresolved, never
